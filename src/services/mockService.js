const products = [
    {
        "id": 1,
        "name": "Rascador para gatos - CanCat",
        "price": 25000,
        "category": "Gatos",
        "stock": "24",
        "imgurl": "/img/products/product1.webp",
        "detail": ""
    },
    {
        "id": 2,
...
const getProductsByCategory = (categoryid) => {
  return new Promise ((resolve, reject) => {
      let reqProducts = products.filter(product => product.category === categoryid);

      setTimeout(() =>{
          if (reqProducts)
            resolve(reqProducts);
          else
              reject("No se encontro esa categoria.");
      }, 2000);
  });
}
...
const getProduct = (id) => {
  return new Promise((resolve, reject) => {
    const reqProduct = products.find((product) => {return (product.id === parseInt(id))})
